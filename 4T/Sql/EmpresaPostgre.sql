CREATE TABLE IF NOT EXISTS tbFabricante (faId INTEGER PRIMARY KEY,
                faNombre VARCHAR(50) NOT NULL,
                faPais VARCHAR(50) NOT NULL
                );

CREATE TABLE IF NOT EXISTS tbComercio (coCif INTEGER PRIMARY KEY,
                coNombre VARCHAR (50) NOT NULL,
                coCiudad VARCHAR (50) NOT NULL
                );

CREATE TABLE IF NOT EXISTS tbCliente (clDni INTEGER PRIMARY KEY,
                clNombre VARCHAR(50) NOT NULL,
                clEdad SMALLINT NOT NULL
                );

CREATE TABLE IF NOT EXISTS tbPrograma (prCodigo INTEGER PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS tbDesarrolla (faId INTEGER,
...
CREATE TABLE IF NOT EXISTS tbDistribuye (coCif INTEGER,
                FOREIGN KEY (coCif) REFERENCES tbComercio(coCif) ON DELETE CASCADE ON UPDATE CASCADE,
                prCodigo INTEGER,
                FOREIGN KEY (prCodigo) REFERENCES tbPrograma(prCodigo) ON DELETE CASCADE ON UPDATE CASCADE,
                diCantidad INTEGER NOT NULL,
                PRIMARY KEY (coCif, prCodigo)
                );

CREATE TABLE IF NOT EXISTS tbRegistra (coCif INTEGER,
                FOREIGN KEY (coCif) REFERENCES tbComercio(coCif) ON DELETE CASCADE ON UPDATE CASCADE,
                clDni INTEGER,
